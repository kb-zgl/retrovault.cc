...
export default defineEventHandler(async (event) => {
  ...
  const platformRows = await sqlAll('SELECT DISTINCT platform FROM games ORDER BY platform')
  const genreRows = await sqlAll('SELECT DISTINCT genre FROM games WHERE genre IS NOT NULL AND genre != "" ORDER BY genre')
